...
const getRandomItem = last => {
	...
	if (last) {
		while (step2list[randomPos] !== last) {
			let selectedItem = step2list[randomPos];
			return selectedItem;
		}
	} else {
		return step2list[randomPos];
	}
};
		...
		<View style={styles.container} onLayout={onLayout.bind(this)}>
			...
			<ScrollView>
				...
				<View style={styles.answerContainer}>
					...
					<Text style={styles.title}>{splitText(step.answerText, step.correctText)[1]}</Text>
				</View>
